...
from jax_guam.utils.jax_utils import tree_add, tree_inner_product, tree_mac

_State = TypeVar("_State")


def ode2(xdot: Callable, dt: float, state: _State) -> _State:
    """Heun's method."""
    state0 = state
    k0 = xdot(state0)
...
def ode3(xdot: Callable, dt: float, state: _State) -> _State:
...
def ode4(xdot: Callable, dt: float, state: _State) -> _State:
    """RK4."""
    state0 = state
    k0 = xdot(state0)
